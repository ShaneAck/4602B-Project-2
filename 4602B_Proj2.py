...
# All_Redshift_std = np.sqrt(np.sum(All_Redshift_ER-np.mean(All_Redshift_ER))/np.size(All_Redshift_ER))
# All_appMAG_std = np.sqrt(np.sum(All_appMAG_ER-np.mean(All_appMAG_ER))/np.size(All_appMAG_ER))

# Note script M subscript B is called the modified absolute magnitude

# QUESTION (1): Shane 

def appmag(z, mod_absmag):
    '''Equation (2) from assignment'''
    return mod_absmag + 5*np.log10(c*z)

popt, pcov = curve_fit(appmag,
                data1999tab2['redshift'],
                data1999tab2['appmagcorr'],
                sigma=data1999tab2['appmagcorrerr'],
                absolute_sigma=True)
mod_absmag = popt[0]
mod_absmag_std = np.sqrt(np.diag(pcov))
fig, ax = plt.subplots()
...
ax.plot(np.sort(data1999tab2['redshift']),
        appmag(np.sort(data1999tab2['redshift']), mod_absmag),
        label='$m_B(z)$ for $\mathscr{M}_B=%.2f$' % mod_absmag,
        c='g',
        zorder=0)
plt.legend()
...
def mB_model(z, Omega_M0, Omega_Lambda0):
    ...
    return mod_absmag + 5 * np.log10(D_L)

bounds = (0, 1)
initial_guess = (0.3, 0.7)

popt, pcov = curve_fit(mB_model, All_Redshift, All_appMAG, p0=initial_guess, bounds=bounds) #sigma=All_appMAG_std, absolute_sigma=True)
print(pcov)
Omega_M0_T = popt[0]
Omega_M0_T_std = np.sqrt(pcov[0,0])
Omega_Lambda0_T = popt[1]
Omega_Lambda0_T_std = np.sqrt(pcov[1,1])
...
def forced_mB_model_flat_universe(z, Omega_M0):
    ...
    return mod_absmag + 5 * np.log10(DL)

bounds = (0, 1)
initial_guess = [0.3]

popt, pcov = curve_fit(forced_mB_model_flat_universe,
                       All_Redshift, All_appMAG,
                       p0 = initial_guess,
                       bounds = bounds)
# Note that including bounds, initial_guess does not
# affect our results. We include it anyways for an
# extra layer of ensurance.

Omega_M0_T_FL = popt[0]
Omega_M0_T_FL_std = np.sqrt(np.diag(pcov))
Omega_Lambda0_T_FL = 1 - Omega_M0_T_FL
Omega_Lambda0_T_FL_std = Omega_M0_T_FL_std
...
appMAG_Original = mB_model(dense_redshifts, Omega_M0_T_FL, Omega_Lambda0_T_FL)

cases = [[1,0],[0.5,0.5],[0,1]]
All_Matter = mB_model(dense_redshifts, cases[0][0], cases[0][1])
Equal_Case = mB_model(dense_redshifts, cases[1][0], cases[1][1])
...
#            label='Perlmutter et al. (1999)',
#            c='blue', linewidth=0.3, zorder=5)
ax.errorbar(All_Redshift, All_appMAG,
            xerr=All_Redshift_ER, yerr=All_appMAG_ER,
            label='Perlmutter et al. (1999)',
            fmt='o', linewidth=1, capsize=3, zorder=4, color='blue')
ax.plot(dense_redshifts, appMAG_Original,
        label='$(\Omega_{M,0}, \Omega_{\Lambda,0})=(%.2f, %.2f)$' % (Omega_M0_T_FL, Omega_Lambda0_T_FL),
        c='black', zorder=3)
